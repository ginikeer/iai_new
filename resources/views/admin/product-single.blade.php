		 	...
		 	<div class="panel-body">
				...
				<form class="form-horizontal" id="product-form" method="post" action="{{ url('admin/product/save') }}" role="form">
					...
					<div class="form-group">
						...
						<div class="col-sm-2">
							<select class="form-control" name="tag_id" id="case-category">
								@foreach ($tag as $c)
								<option value="{{ $c->id }}" @if(isset($p->tag_id)) {{$p->tag_id==$c->id ? 'selected' : ''}}  @endif>{{ $c->title }}</option>
								@endforeach
							</select>
						</div>
						<div class="col-sm-4">
							...
							<div class="multiple-select related-case related-case-{{ $tag[$i]->id }}" @if($related_case["cid_" . $tag[$i]->id]['titles']) style="display: block;" @else @if($i != 0) style="display: none;" @endif @endif>
		                        ...
		                        <ul class="select-list">
		                        	<!-- 遍历每个标签下的案例信息 -->
		                        	@foreach ( $cases["cid_" . $tag[$i]->id] as $r )

	                        		<li>
                        				<input name="case_select_{{ $tag[$i]->id }}" data-str="{{ $r->title }}" type="checkbox" value="{{ $r->id }}"
                        					@if(in_array($r->id, $related_case["cid_" . $tag[$i]->id]['ids'])) checked @endif />
                        				<span>{{ $r->title }}</span>
	                        		</li>
	                        		@endforeach
	                        		<li>
	                        			<div data-input-name="case_select_{{ $tag[$i]->id }}" class="btn btn-success">确定</div>
	                        			<input type="hidden" name="case_ids[]" value="{{ implode(',', $related_case['cid_' . $tag[$i]->id]['ids']) }}" />
	                        		</li>
	                        	</ul>
							</div>
							@endfor
